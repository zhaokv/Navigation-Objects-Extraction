        ...
        for linkCluster in linkClusterList:
            ...
            for link in linkCluster:
                if 'cluster' in link.attrs:
                    tmp.append([clusterSize, textNumAvg, clusterChaotic])
                    label = labelDic[link['cluster']]
                    tmp_g.append(int(label <= 1))
                #clusterIndex.append(int(link['clusterindex']))
        linkMatrix.append(tmp)
        groundList.append(tmp_g)
    return (linkMatrix, groundList, clusterIndex)

def main():
    htmlFileDir = '../../data/cleanEval'
    htmlFileDir = '../../data/SSD/Big5_/techweb.com'
    ...
    job_n = 16
    trainRatioList = [float(i)/100 for i in range(10, 101, 10)]
    for trainRatio in trainRatioList:
        ...
        while turn < turnNum:
            try:
                ...
                testBound = int(testRatio*len(dataList))
                upperBound = int(trainRatio*(1-testRatio)*len(dataList))
                tlinkMatrix = [dataList[i][0][j] for i in range(0, upperBound) for j in range(0, len(dataList[i][0]))]
                tgroundList = [dataList[i][1][j] for i in range(0, upperBound) for j in range(0, len(dataList[i][1]))]
                scaler = StandardScaler()
                tlinkMatrix = scaler.fit_transform(tlinkMatrix)
                grid=None
                if search:
                    C_range = np.logspace(-2, 10, 13)
                    gamma_range = np.logspace(-9, 3, 13)
                    param_grid = dict(gamma = gamma_range, C = C_range)
                    cv = StratifiedShuffleSplit(tgroundList, n_iter = 5, test_size = 0.2, random_state = 42)
                    grid = GridSearchCV(SVC(), param_grid = param_grid,  cv = cv, n_jobs = job_n)
                    grid.fit(tlinkMatrix, tgroundList)
                    clf = SVC(C = grid.best_params_['C'], gamma = grid.best_params_['gamma'])
                else:
                    C = float(upperBound/sum(tgroundList))
                    clf = SVC(C = 1, gamma = 0.10, kernel='rbf')
                clf.fit(tlinkMatrix, tgroundList)
                plinkMatrix = [dataList[i][0][j] for i in range(testBound, len(dataList)) for j in range(0, len(dataList[i][0]))]
                pgroundList = [dataList[i][1][j] for i in range(testBound, len(dataList)) for j in range(0, len(dataList[i][1]))]
                plinkMatrix = scaler.transform(plinkMatrix)
                predict = clf.predict(plinkMatrix)
                tmp = metric(predict, pgroundList)
                precision += tmp[0]
                recall += tmp[1]
                f1_score += tmp[2]
                accuracy += tmp[3]
