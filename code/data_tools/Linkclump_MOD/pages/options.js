...
$(function() {
	...
	$('li').click(function(){
		$(this).find('input[name="category"]').attr("checked", "checked");
	})
	...
	document.getElementById('cancel').addEventListener('click', cancel_links);
	document.getElementById('save').addEventListener('click', save_links);
...
function save_links(){
	...
	if(!$('input[name="category"]:checked').val()){
		...
		catelog = $('input[name="category"]:checked').val();
	}
	
	chrome.extension.sendMessage({
		message: 'save_links',
		result: { save:true, value:catelog }
	});
}
...
function cancel_links(){
	var cancel = "0";
	chrome.extension.sendMessage({
		message: 'cancel_links',
		result: {save:false, value: cancel }
	});
}
